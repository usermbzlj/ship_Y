...
- 在太阳邻域星图中选择出发地和目的地，签发自然语言最高指令。
- 以暂停、`1×`、`60×`、`3,600×`、`21,600×` 推进模拟时间。
...
- 通过人工干预面板触发预设异常（包括真实居住环轴承劣化），或用“原力”直接修改受支持的物理量。
- 保存和恢复包含引擎、随机流、事件队列、干预记录与全部人员状态的本地快照。
- 在安全抵达后结束航程，查看客观结果和 `6` 名确定性分层代表的主观评价；代表覆盖船员/乘客、体验高低端及重大事故亲历者，项目不提供统一分数。

界面包含航程、舰体、乘员、AI 观察和人工干预五个主要视图，采用适配笔记本与 `1920×1080` 的二维工业化信息界面，不依赖重型 3D 引擎。
...
| 上帝模式 | 外部写入先在草稿状态中原子校验；成功与拒绝都会审计，并记录声明的质量、能量、线动量和角动量差额；轴承劣化事件改变真实 A/B 环轴承条件，后续摩擦、振动、耗能和废热由求解器演化 |
...
| 本地存档 | 本地存档封装 `v18` 内含运行时快照 `v15` 和关键乘客轮询快照 `v2`；手动保存先暂停新步进/模型调用，等待在途物理事务原子完成再取 Worker 安全点；除权威世界外还保存轮询游标、预算、含固定舱区观测的 `published`/`pending`、个人调度及本人私人 note，加载后继续原计划；聚合核心、人员、舱室、冷却、电力、导航、旋转、水、维修子域快照分别为 `v6`、`v2`、`v3`、`v5`、`v4`、`v5`、`v1`、`v1`、`v1` |
| 游戏界面 | 起航流程、状态总览、舰体分区、乘员、AI 状态、人工干预和结束报告；全舰事件时间线最多保留最近 `500` 条 |
...
不配置 LLM 也可以启动界面、运行确定性模拟、使用存档和人工干预；AI 页面会把缺少密钥的岗位标为 `missing-secret`，舰长自动决策不会启动。
...
该启动器会在内存中构造完整 `40` 槽配置，把同一密钥映射到八个部门及其关键乘客槽位，并启用 OpenAI 兼容消息序列化；它不会生成含密钥的 dotenv 或 JSON 文件。也可直接调用 `scripts/start-deepseek.ps1`，用 `-Port`、`-Model deepseek-v4-flash|deepseek-v4-pro` 和 `-Thinking enabled|disabled` 覆盖默认值。终端只显示供应商域名、模型与槽位数，不打印密钥。
...
```json
{
  "endpoint": {
    ...
    "headers": {
      "x-ship-client": "far-horizon/local"
    },
    "secretHeaders": [
      {
...
9. 玩家可以只观察，也可以进入人工干预模式，从世界外制造变量，检验舰长的诊断与恢复能力。
...
```text
app/
  mission-control.tsx        游戏界面、Worker 桥接与当前舰长协调
...
- 生产构建能够服务真实任务界面，并包含 Worker-backed 模拟器。

## 数据、安全与费用

- 所有游戏存档目前保存在当前浏览器配置文件的 `localStorage` 中，不上传到项目服务器，但也**没有加密**。能访问该浏览器配置文件的人可以读取或删除它。
...
- 模型只能获得调用中明确提供的最高指令、事件和授权观测。它不会自动读取 Worker 内的完整世界真值或玩家上帝操作说明。
...
`.env.local` 已被 Git 忽略；提交前仍应检查变更，确认没有把密钥复制到源码、配置示例、日志或存档中。
