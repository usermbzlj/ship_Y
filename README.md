...
- 在上帝视图用自然语言描述干预意图：独立配置的**上帝助手**（`playerAssistants.godAssistant`，**不属于**固定 `40` 节点舰内 LLM 拓扑）经 `POST /api/llm/invoke`（`intent: "god-assist"`）编译为因果事件或原力覆写计划，预览后确认执行；若物理引擎拒收，会据拒收原因自动向 LLM 修订一次。
...
| 上帝模式 | 外部写入先在草稿状态中原子校验；成功与拒绝都会审计，并记录声明的质量、能量、线动量和角动量差额；轴承劣化事件改变真实 A/B 环轴承条件，后续摩擦、振动、耗能和废热由求解器演化；UI 对因果事件与原力覆写要求二次确认；上帝视图另提供玩家侧**上帝助手**（`playerAssistants.godAssistant`，独立于 `40` 节点舰内拓扑）做 NL→计划→确认→执行，物理拒收时最多自动修订一次 |
| 程序化事件 | React 层确定性调度器（固定种子，当前未写入存档）；七类事件中微流星体、冷却泵卡死、恒星耀斑经 `environment:procedural` 注入 Worker；其余类型以时间线叙事为主 |
| 固定 LLM 拓扑 | `8` 个部门 actor 与 `32` 个不可替换的关键乘客 LLM 槽位组成唯一合法的 `40` 节点舰内拓扑；生产路径会调用舰长、按需部门顾问和符合条件的关键乘客；运行时不能创建、克隆、删除或提升代理；玩家侧上帝助手（`playerAssistants.godAssistant`）在拓扑外单独配置 |
...
该跨平台启动器（`scripts/start-deepseek.mjs`）会在内存中构造完整 `40` 槽舰内配置，并把同一密钥映射到八个部门、关键乘客槽位，以及**玩家侧** `playerAssistants.godAssistant`（上帝助手，不在 `40` 节点拓扑内）；同时设置 `CLOUDFLARE_INCLUDE_PROCESS_ENV=true` 以便 vinext/Cloudflare 开发运行时转发进程环境变量。它不会生成含密钥的 dotenv 或 JSON 文件。可用参数示例：
...
默认八个舰内岗位与玩家侧上帝助手使用 `.env.example` 中的这些变量：
...
当前 UI 以规范 `40` 节点舰内拓扑全部通过配置校验作为自动闭环的就绪条件。默认配置会从八部门模板显式扩展出 `32` 个关键乘客槽位，这些槽位复用乘客服务端点，因此仍只需填写上列八个部门密钥；同一供应商允许多个岗位引用同一个实际密钥，但仍建议保留不同的环境变量名，便于独立计费、限流和撤销。

**上帝助手**单独配置在 `LLM_CONFIG_JSON` / `config/llm.example.json` 的 `playerAssistants.godAssistant` 下，使用 `SHIP_GOD_ASSIST_LLM_API_KEY`。它只帮玩家把自然语言编译为上帝干预计划，**不是**舰长、部门或关键乘客，也不计入 `40` 节点拓扑。未配置时仍可用手动因果/原力面板；`npm run check:env` 会一并检查该密钥与 `godAssistant` 端点是否就绪。
...
app/
  ...
  ui/
    ...
    components/                         星图、拓扑、API 状态、上帝助手面板等
...
lib/
  ...
  llm/god-assist.ts                     玩家侧上帝助手（拓扑外）计划编译与网关
...
- 玩家侧上帝助手（`intent: "god-assist"`）计划编译、`playerAssistants.godAssistant` 配置校验，以及发电原力覆写后的电网对账回归；
